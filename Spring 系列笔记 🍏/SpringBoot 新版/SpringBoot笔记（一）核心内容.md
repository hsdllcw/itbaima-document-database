...
public void process(AnnotationMetadata annotationMetadata, DeferredImportSelector deferredImportSelector) {
    ...
    //获取所有的Entry，其实就是读取spring.factories来查看有哪些自动配置类
...
通过断点调试得到这个位置解析出来的全部自动配置类：

![image-20230719020152801](https://s2.loli.net/2023/07/19/REugdUCT6fQ9hNA.png)
...
经过这一套流程，简而言之就是SpringBoot读取`spring.factories`文件来确定要加载哪些自动配置类来实现的全自动化，真正做到添加依赖就能够直接完成配置和运行，至此，SpringBoot的原理部分就探究完毕了。
...
接着再编写`spring.factories`文件，并将我们的自动配置类添加即可：
